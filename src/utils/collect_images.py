...
class CaptureImages(): 
    ...
    def run(self, sleep_time: int = 1, num_images: int = 10):
        ...
        for class_idx, img_class in enumerate(self.classes): 
            ...
            # 15-second pause between classes (skip after the last class)
            if class_idx < len(self.classes) - 1:
                logger.info(f"Pausing 15 seconds before next class...")
                for countdown in range(15, 0, -1):
                    ret, frame = self.cap.read()
                    if ret:
                        cv2.putText(frame, f'Next class in: {countdown}s', (0, 100),
                                    cv2.FONT_HERSHEY_DUPLEX, 3, (0, 255, 0), 2, cv2.LINE_AA)
                        cv2.imshow('Image Capture', frame)
                    if cv2.waitKey(1000) & 0xFF == ord('q'):
                        logger.warning("Quit key pressed -- stopping session.")
                        self.cap.release()
...
if __name__ == '__main__': 
    cap = CaptureImages('./data/train/images', classes, 0) 
    cap.run(num_images=60, sleep_time=0.1)
